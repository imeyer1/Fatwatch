...
def customers():
    ...
    headings = [item.replace("cust_","") for item in headings] #change for other tables
    ...
    for e in list(newData):
        ...
        for x in l:
            if isinstance(x, datetime):
                pos=l.index(x)
                d = x.strptime(str(x),"%Y-%m-%d %H:%M:%S.%f").strftime( "%d/%m/%Y %H:%M:%S")
                l[pos]= d
                newData2.append(l)
    return render_template('customers.html', title='Contacts', headings= headings, data=newData2, username=current_user.usr_name)
...
def ajax_update():
    ...
    if request.method == 'POST':
        ...
        for d in data:
                ...
                if re.findall(" \d\d/\d\d/\d\d\d\d", d):
                    string= datetime.strptime(d.strip(),"%d/%m/%Y %H:%M:%S").strftime( "%Y-%m-%d %H:%M:%S.%f")
        ...
        cur.execute("UPDATE customers SET cust_name = %s, cust_address = %s, cust_zip = %s,cust_city = %s,cust_email = %s,cust_phone = %s, cust_active = %s, cust_timestamp = %s WHERE cust_id = %s ", val)
        conn.commit()       
        cur.close()
