...
def customers():
    ...
    headings=column_names = [row[0] for row in cur row.repalce("cust_","")]
    cur.execute("SELECT * FROM customers;")
    data = cur.fetchall()
    newData = [tuple(s if s != None else " " for s in tup) for tup in data]
    
    return render_template('customers.html', title='Contacts', headings= headings, data=newData, username=current_user.usr_name, form=form)
...
def ajax_update():
    ...
    if request.method == 'POST':
        ...
        txtactive = request.form['txtactive']
        print(string)
        cur.execute("UPDATE customers SET cust_name = %s, cust_address = %s, cust_zip = %s,cust_city = %s,cust_email = %s,cust_phone = %s, cust_active = %s WHERE id = %s ", [txtname, txtaddress,txtzip, txtcity, txtemail, txtphone,txtactive, string])
        conn.commit()       
        cur.close()
