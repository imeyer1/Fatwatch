    ...
    if request.method == 'POST':
        ...
        for d in data:
                ...
                if d:
                    ...
                    string = d
                val.append(string)
        #cur.execute("UPDATE "+title+ "SET cust_name = %s, cust_address = %s, cust_zip = %s,cust_city = %s,cust_phone = %s,cust_email = %s, cust_active = %s, cust_timestamp = %s WHERE cust_id = %s ", val)
        #set the ID value to last position
        val.append(val.pop(0))
        query= "UPDATE " + str(title)+ " SET " + columns + "= %s WHERE "+str(prefix)+"id = %s"
...
def delete():
    ...
    prefix = request.form['prefix'].strip()
    print(title+" "+id+" "+prefix)
    cur = conn.cursor()
    if request.method == 'POST':
        query= "DELETE FROM " + str(title)+ " WHERE "+str(prefix)+"id = "+ id
        cur.execute(query)
        conn.commit()       
        cur.close()
