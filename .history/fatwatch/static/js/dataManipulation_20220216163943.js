...
$(document).on('click',".btn_new", function(event)
{
    ...
    if (rows > 1)
    {
        ...
        var td_new='<tr class="table-row" row_id="{{row[0]}} ">'
        for(var c=0;c <= colCount; c++)
        {
            td_new +='<td><div input readonly="false" class="row_data" edit_type="click">null</div></td><td>'
        }
        var line=  $('tbody').find('tr').append(td_new);
        console.log(line)
        var buttons = '<td><a id="btn_edit" class="btn_edit" row_id="{{row[0]}}"><i class="fa fa-pencil fa-1x"></i></a><a id="btn_save" class="btn_save" row-id="{{row[0]}}"><i class="fa fa-check fa-1x"></i></a><a id="btn_cancel" class="btn_cancel" row-id="{{row[0]}}"><i class="fa fa-xmark fa-1x"></i></a><a id="btn_delete" class="btn_delete" row-id="{{row[0]}}"><i class="fa fa-trash-o fa-1x"></i></a></td>"'
        $('tbody').find('tr').append(buttons);
        console.log(buttons)
       $('tr').find('.row_data').not(':first').not(':last')
       .attr('contenteditable', 'true')
       .attr('edit_type', 'button')
       .addClass('bg-info')
       .css('padding','3px')
        var buttons = '<td><a id="btn_edit" class="btn_edit" row_id="{{row[0]}}"><i class="fa fa-pencil fa-1x"></i></a><a id="btn_save" class="btn_save" row-id="{{row[0]}}"><i class="fa fa-check fa-1x"></i></a><a id="btn_cancel" class="btn_cancel" row-id="{{row[0]}}"><i class="fa fa-xmark fa-1x"></i></a><a id="btn_delete" class="btn_delete" row-id="{{row[0]}}"><i class="fa fa-trash-o fa-1x"></i></a></td>"'
        console.log(buttons)
        // td_new.concat(td_new)
        // $('#table').find('tbody').append(td_new);
        }
});
