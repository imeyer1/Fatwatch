...
def customers():
    ...
    newData2 = [tuple(newData[7].lower() for s in tup) for tup in newData]
    newData3 = newData2[7].lower()
    data=tuple(newData3)

    return render_template('customers.html', title='Contacts', headings= headings, data=data, username=current_user.usr_name, form=form)
