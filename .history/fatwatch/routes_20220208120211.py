...
def customers():
    ...
    #columns
    cur.execute("SELECT (each(hstore(customers))).*FROM    customers ORDER BY cust_id")
    data = cur.fetchall()
    #modifies None in ""
    ...
    for e in values:
        # l=list(e)
        # for x in e:
            if re.match("\d\d\d\d-\d\d-\d\d",e):
                # pos=e.index(e)
                e = datetime.strptime(e,"%Y-%m-%d %H:%M:%S").strftime( "%d/%m/%Y %H:%M:%S")
                # e[pos]= d
            newData2.append(e)
